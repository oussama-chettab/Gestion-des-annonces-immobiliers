...
<body>
    ...
    <h2 style="text-align: center; margin-top: 30px;">Mes annonces</h2>
    <div class="annonce_valide">
        <?php 
            $Id_U = $_SESSION['Id_U'];
            $query_info = mysqli_query($conn, "SELECT * FROM annonce WHERE id_U='$Id_U'; ");
            while($data=mysqli_fetch_array($query_info)){
                $photo = $data['photo'];
                //$imagePath = 'annonce_images/' .$photo;
                $photo_base64 = base64_encode($photo);
                //$photo_base64 = $data['photo'];
                $imagePath = 'annonce_images/' . basename($photo_base64);
        ?>
        
        <div class="annonce">
            <div class="supp">
                <h3><a href="<?php echo $url; ?>"> <?php echo $data['type_annonce'].' '.$data['type_bien'] ;?></a></h3><br>
                <form method="POST" action="Suppr_mesAnn.php">
